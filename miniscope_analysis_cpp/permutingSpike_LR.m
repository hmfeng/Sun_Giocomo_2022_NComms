function [PlaceCells,Tinfo,infoScoreThresh] = permutingSpike_LR(neuron,behav,thresh,deltaTall,occThresh,nboot,binsize,Stype,Method)
...
[firingrateAll,countAll,countTime] = calculate_subset_ratemap(neuron,behav,thresh,binsize);
...
for nE = 1:nboot
    ...
    [firingrateAll,countAll,countTime] = calculate_subset_ratemap(neuronboot,behav,thresh,binsize);
    for jj = 1:length(firingrateAll)
        MeanFiringRateAll= sum(sum(countAll{1,jj}))/sum(sum(countTime));
